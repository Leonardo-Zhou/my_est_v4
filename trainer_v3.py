...
class Trainer:
    ...
    def nonlambertian_decompose(self, inputs, outputs):
        ...
        # 批量分解和重建
        for f_i in self.opt.frame_ids:
            albedo, shading, _ = self.models["decompose"](decompose_features[f_i])
            specular = inputs[("color_aug", f_i, 0)] - albedo * shading
    ...
    def compute_losses(self, inputs, outputs):
        ...
        # 修改3.1:增加PPS对shading的约束。
        if self.opt.start_pps_epoch <= self.epoch:
            loss_pps_shading += (outputs[("shading", 0, 0)] - outputs[("PPS", 0, 0)]).abs().mean()
            for frame_id in self.opt.frame_ids[1:]:
                loss_pps_shading += (outputs[("shading_adjust_warp", 0, frame_id)] - outputs[("PPS", 0, frame_id)]).abs().mean()
        ...
        total_loss = (
            ...
            self.opt.pps_shading * loss_pps_shading / num_all_frames  # 新增shading平滑损失权重
        )
