...
class Trainer:
    ...
    def process_batch(self, inputs):
        ...
        for frame_id in self.opt.frame_ids[1:]:
            f_t = self.models['encoder'](inputs[("color_aug", frame_id, 0)])
            o_t = self.models['depth'](f_t)
            outputs[("disp", frame_id)] = o_t[("disp", 0)]
        # pose estimation
    ...
    def nonlambertian_decompose(self, inputs, outputs):
        ...
        for f_i in self.opt.frame_ids:
            ...
            # shading = (inputs[("color_aug", f_i, 0)] - specular) / (albedo + 1e-8)
            shading = shading.mean(dim=1, keepdim=True)
            outputs[("albedo", 0, f_i)] = albedo
            # 修改4.2: 使用PPS优化（shading = (I - specular) / albedo, shading取三维的平均值）
            outputs[("shading", 0, f_i)] = shading
            outputs[("specular", 0, f_i)] = specular
            outputs[('reconstruction_color', 0, f_i)] = albedo * shading + specular
        ...
        if self.opt.start_pps_epoch <= self.epoch:
            for frame_id in self.opt.frame_ids:
                ...
                pps_params_t = calculate_pps(1.0/depth_t, inputs[("color_aug", frame_id, 0)], inputs[("K", 0)][:, :3, :3], self.light, self.light_dir, None)
                lambertian_result_t = calculate_lambertian(pps_params_t, outputs[("albedo", 0, frame_id)])
                outputs[("PPS", 0, frame_id)] = lambertian_result_t['PPS']
                # 修改4.1:使用PPS替代shading
                outputs[("shading", 0, frame_id)] = outputs[("PPS", 0, frame_id)]
                if frame_id == 0:
                    outputs[('reconstruction_color', 0, 0)] = lambertian_result_t['lambertian'] + outputs[("specular", 0, 0)]
        ...
        for i, frame_id in enumerate(self.opt.frame_ids[1:]):
            ...
            if '1.1' in self.opt.change_type:
                outputs[("reprojection_color_warp_lambertian", 0, frame_id)] = (
    ...
    def compute_losses(self, inputs, outputs):
        ...
        loss_pps_shading = torch.tensor(0.0, device=device)
            ...
            if mask_sum > 0:  # 避免除零错误
                loss_reprojection += (
                    self.compute_reprojection_loss(
                        ...
                        outputs[("reprojection_color_warp_lambertian", 0, frame_id)]  # warp+调整后的重建图像
                    ) * mask  # 只计算有效区域的损失
                ).sum() / mask_sum
        ...
        # 修改4.2: 增加PPS-Shading 损失。仅在高纹理区域令Shading趋近PPS
        for frame_id in self.opt.frame_ids:
            # 计算图像梯度来识别高纹理区域
            I = inputs[("color_aug", frame_id, 0)]  # 获取彩色图像
            
            # 计算图像梯度（使用与get_smooth_loss相同的方法）
            grad_img_x = torch.mean(torch.abs(I[:, :, :, :-1] - I[:, :, :, 1:]), 1, keepdim=True)  # (b, 1, h, w-1)
            grad_img_y = torch.mean(torch.abs(I[:, :, :-1, :] - I[:, :, 1:, :]), 1, keepdim=True)  # (b, 1, h-1, w)
            
            # 将梯度图填充到原始尺寸（使用零填充）
            grad_img_x = F.pad(grad_img_x, (0, 1), mode='constant', value=0)  # (b, 1, h, w)
            grad_img_y = F.pad(grad_img_y, (0, 0, 0, 1), mode='constant', value=0)  # (b, 1, h, w)
            
            # 计算总梯度幅值
            grad_magnitude = torch.sqrt(grad_img_x**2 + grad_img_y**2 + 1e-8)  # (b, 1, h, w)
            
            # 设置纹理阈值，识别高纹理区域（可以根据需要调整阈值）
            texture_threshold = 0.1
            texture_mask = (grad_magnitude > texture_threshold).float()  # 高纹理区域为1，低纹理区域为0
            
            # 仅在高纹理区域计算PPS-Shading损失
            shading_diff = torch.abs(outputs[("shading", 0, frame_id)] - outputs[("PPS", 0, frame_id)])
            
            # 使用纹理掩码，只计算高纹理区域的损失
            loss_pps_shading += (shading_diff * texture_mask).mean() / (texture_mask.sum() + 1e-8)
        ...
        total_loss = (
            ...
            self.opt.pps_shading * loss_pps_shading / num_all_frames  # PPS-Shading 损失权重
        )
        ...
        losses["pps_shading_loss"] = loss_pps_shading  # PPS-Shading 损失
    ...
    def log(self, mode, inputs, outputs, losses):
        ...
        for j in range(min(4, self.opt.batch_size)):
            ...
            if ("albedo_shading", 0, 0) in outputs:
                writer.add_image("PPS_Albedo/{}".format(j), outputs[("albedo_shading", 0, 0)][j].data, self.step)
