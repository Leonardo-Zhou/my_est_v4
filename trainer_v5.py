...
class Trainer:
    def __init__(self, options):
        ...
        # 修改1.5：增加光源
        self.light = torch.tensor([0.0, 0.0, -1], dtype=torch.float32, device=self.device)
    ...
    def nonlambertian_decompose(self, inputs, outputs):
        ...
        for i, frame_id in enumerate(self.opt.frame_ids[1:]):
            ...
            # )
            if ('1.1' in self.opt.change_type) or ('5.1' in self.opt.change_type):
                outputs[("reprojection_color_warp", 0, frame_id)] = (
                    outputs[("albedo_warp", 0, frame_id)] * outputs[("shading_adjust_warp", 0, frame_id)]
                )
            else:
        ...
        if self.opt.start_pps_epoch <= self.epoch:
            for frame_id in self.opt.frame_ids[:1]:
                disp_t = outputs[("disp", frame_id)]
                disp_t = F.interpolate(disp_t, [self.opt.height, self.opt.width], mode="bilinear", align_corners=True)
                _, depth_t = disp_to_depth(disp_t, self.opt.min_depth, self.opt.max_depth)
                pps_params = calculate_pps(1.0/depth_t, inputs[("color_aug", frame_id, 0)], inputs[("K", 0)][:, :3, :3], self.light, self.light_dir, ori_shape=None)
                lambertian_result = calculate_lambertian(pps_params, outputs[("albedo", frame_id, 0)])
                outputs[("PPS_Albedo", frame_id, 0)] = lambertian_result['lambertian']
                outputs[("PPS", frame_id, 0)] = lambertian_result['PPS']
                outputs[("pps_params", frame_id, 0)] = pps_params
            if self.opt.change_type == '5.1':
                pass
            elif '5.2' in self.opt.change_type:
                for frame_id in self.opt.frame_ids[1:]:
                    outputs[("reprojection_color_warp_nonlambertian", 0, frame_id)] = (
                        outputs[("albedo_warp", 0, frame_id)] * outputs[("PPS", 0, 0)]
                    )
    ...
    def compute_losses(self, inputs, outputs):
        ...
        loss_pps_corr = torch.tensor(0.0, device=device)
        ...
        for frame_id in self.opt.frame_ids[1:]:
            ...
            mask_sum = mask.sum()
            if mask_sum > 0:  # 避免除零错误
                if '5.1' in self.opt.change_type:
                    loss_reprojection += (
                        self.compute_reprojection_loss(
                            outputs[("albedo_shading", 0, 0)],   # 当前帧的真实图像（目标）
                            outputs[("reprojection_color_warp", 0, frame_id)]  # warp+调整后的重建图像
                        ) * mask  # 只计算有效区域的损失
                    ).sum() / mask_sum
                elif '5.2' in self.opt.change_type:
                # 修改5.2: 使用PPS作为shading
                    loss_reprojection += (
                        self.compute_reprojection_loss(
                            outputs[("PPS_Albedo", 0, 0)],   # 当前帧的真实图像（目标）
                            outputs[("reprojection_color_warp_nonlambertian", 0, frame_id)]  # warp+调整后的重建图像
                        ) * mask  # 只计算有效区域的损失
                    ).sum() / mask_sum
        ...
        # 修改5.1: 增加PPS*Albedo 与 I - Specular 的CORR损失
        def cal_pps_corr_loss(image_tensor, pps_albedo_tensor, threshold=1.0):
            """
            直接计算两个(b,3,h,w)tensor之间的PPS-corr损失
            
            参数:
                image_tensor: 原始图像，形状为(B, 3, H, W)
                pps_albedo_tensor: PPS*Albedo结果，形状为(B, 3, H, W)
                threshold: 高光阈值，默认为0.98
            ...
            def rgb_to_grayscale(input_tensor):
                """将RGB图像转换为灰度图"""
                weights = torch.tensor([0.2989, 0.5870, 0.1140], device=input_tensor.device)
                return torch.matmul(input_tensor.permute(0, 2, 3, 1), weights).unsqueeze(1)

            # 将两个RGB图像都转换为灰度图
            img_gray = rgb_to_grayscale(image_tensor)  # (B, 1, H, W)
            pps_albedo_gray = rgb_to_grayscale(pps_albedo_tensor)  # (B, 1, H, W)
            
            # 创建高光区域掩码
            specular_mask = (img_gray > threshold).float()
            
            # 应用掩码
            img_masked = img_gray * (1 - specular_mask)
            pps_albedo_masked = pps_albedo_gray * (1 - specular_mask)
            
            # 展平空间维度
            img_flat = img_masked.flatten(start_dim=2)  # (B, 1, H*W)
            pps_flat = pps_albedo_masked.flatten(start_dim=2)  # (B, 1, H*W)
            
            # 计算相关性
            img_mean = img_flat.mean(dim=2, keepdim=True)
            pps_mean = pps_flat.mean(dim=2, keepdim=True)
            
            img_centered = img_flat - img_mean
            pps_centered = pps_flat - pps_mean
            
            # 计算协方差和标准差
            cov = (img_centered * pps_centered).sum(dim=2)
            img_std = img_centered.pow(2).sum(dim=2).sqrt()
            pps_std = pps_centered.pow(2).sum(dim=2).sqrt()
            
            # 计算相关系数
            correlation = cov / (img_std * pps_std + 1e-8)
        ...
        if '5.1' in self.opt.change_type:
        # 计算PPS*Albedo 与 I - Specular 的CORR损失
            for frame_id in self.opt.frame_ids[:1]:
                pps_albedo = outputs[("PPS_Albedo", frame_id, 0)]
                I = inputs[("color_aug", frame_id, 0)]
                specular = outputs[("specular", frame_id, 0)]
                loss_pps_corr += cal_pps_corr_loss(I - specular, pps_albedo)
        ...
        total_loss = (
            ...
            self.opt.pps_corr * loss_pps_corr / num_all_frames  # 新增PPS*Albedo 与 I - Specular 的CORR损失权重
        )
        ...
        losses["shading_smoothness_loss"] = loss_shading_smoothness  # 新增shading平滑损失
        losses["pps_corr_loss"] = loss_pps_corr  # 新增PPS*Albedo 与 I - Specular 的CORR损失
    ...
    def log(self, mode, inputs, outputs, losses):
        ...
        for j in range(min(4, self.opt.batch_size)):
            ...
            if ("PPS_Albedo", 0, 0) in outputs:
                writer.add_image("PPS_Albedo/{}".format(j), outputs[("PPS_Albedo", 0, 0)][j].data, self.step)
            ...
            if ("pps_params", 0, 0) in outputs:
                writer.add_image("A/{}".format(j), outputs[("pps_params", 0, 0)]["A"][j].data, self.step)
                writer.add_image("N/{}".format(j), outputs[("pps_params", 0, 0)]["N"][j].data, self.step)
